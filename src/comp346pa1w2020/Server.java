...
public class Server extends Thread {
    ...
    //private static int servYieldCount = 0;      // counter for server yields
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
         ...
         while ((!objNetwork.getClientConnectionStatus().equals("disconnected")))
         {
             ...
             {                                                      // Added additional check to avoid infinite loop when client already disconnected
                 //servYieldCount++;                            // count # server yields
                 Thread.yield();                                        // While InBuffer is empty, do a context switch with yield
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
                 ...
                 {                                                      // Added additional check to avoid infinite loop when client already disconnected
                     //servYieldCount++;                            // count # server yields
                     Thread.yield();                                        // While OutBuffer is full, do a context switch with yield
    ...
    public void run()
    {
        ...
        //Instant serverStartTime, serverEndTime;     // Changed from default long, valid in Java 8+
        //Duration timeElapsed;                       // Added to track elapsed time, valid in Java 8+
        ...
        //timeElapsed = Duration.between(serverStartTime,serverEndTime);

        //System.out.println("# server yields = " + servYieldCount);
        /* End of implemented code for the run method */

        System.out.println("\n Terminating server thread - " + " Running time " + (serverEndTime - serverStartTime) + " milliseconds");
        //System.out.println("\n Terminating server thread - " + " Running time " + timeElapsed.toMillis() + " milliseconds");
