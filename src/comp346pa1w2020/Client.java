...
public class Client extends Thread {
    ...
    //private static int clYieldCount;                     // client yield counter
    static boolean receiveTerm = false;                 // statically tracks if receive client terminated
    static boolean sendTerm = false;                 // statically tracks if send client terminated
     ...
     */
     ...
     { 
       if (operation.equals("sending"))
       {
           ...
           /*if (!(clYieldCount > 0))
           {                       // helps (but doesn't avoid) from resetting counter on second client thread
               clYieldCount = 0;
           }*/
     ...
     */
     public void sendTransactions()
     {
         ...
         {  
            while ((objNetwork.getInBufferStatus().equals("full")) && (!objNetwork.getServerConnectionStatus().equals("disconnected")))
            {                                                      // Added additional check to avoid infinite loop when server already disconnected
                //clYieldCount++;                            // count # client yields
                Thread.yield();                                        // While InBuffer is full, do a context switch with yield
     ...
     */
     public void receiveTransactions(Transactions transact)
     {
         ...
         {     
             ...
             {                                                      // Added additional check to avoid infinite loop when server already disconnected
                 //clYieldCount++;                            // count # client yields
        	     Thread.yield();                                        // While OutBuffer is empty, do a context switch with yield
    ...
    public void run()
    {
        ...
        //Instant sendClientStartTime = Instant.now();
        //Instant receiveClientStartTime = Instant.now();
        //Instant sendClientEndTime, receiveClientEndTime;
        //Duration sendClientTimeElapsed, receiveClientTimeElapsed;
        ...
        if (getClientOperation().equals("receiving"))
        {
            ...
            //receiveClientTimeElapsed = Duration.between(receiveClientStartTime,receiveClientEndTime);
        ...
        else if (getClientOperation().equals("sending"))
        {
            ...
            //sendClientTimeElapsed = Duration.between(sendClientStartTime,sendClientEndTime);
            System.out.println("\n Terminating send client thread - " + " Running time " + (sendClientEndTime-sendClientStartTime) + " milliseconds");

            sendTerm = true;
        }
        //System.out.println("# client yields = " + clYieldCount);
        ...
        /* End of implemented code for the run method */
    }
}
